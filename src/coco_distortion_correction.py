...
def coco_labels_distort_correction(
    ...
    for img_id in img_ids:
        ...
        for ann in anns:
            ...
            bbox = np.array(
                undistort_points(bbox, distortion_mapping_x, distortion_mapping_y),
                dtype=int,
            )
            seg = np.reshape(
                np.array(ann["segmentation"][0], dtype=float),
                [n_points, 1, 2],
                order="C",
            )
            seg = np.array(
                undistort_points(seg, distortion_mapping_x, distortion_mapping_y),
                dtype=int,
            )
            new_annotation = {
...
def distort_correct_all_images(
    images_directory, camera_parameter_file_path, output_directory
):
    map_x = None
    map_y = None
        ...
        if image_name[-4:] == ".jpg":
            ...
            if map_x is None:
                ...
                map_x, map_y = calc_distortion_mapping(
                    camera_parameter_file_path, width, height
                )
...
def main():
    distortion_correct_entire_coco_directory(
        "/Volumes/jw22g14_phd/fk2018/tunasand/20180805_215810_ts_un6k/processed/image/i20180805_215810/for_iridis/histogram_normalised/no_distortion_correction/not_rescaled/",
        ...
        "/Volumes/jw22g14_phd/fk2018/tunasand/20180805_215810_ts_un6k/configuration/camera_parameters_unagi6k.yml",
    )
